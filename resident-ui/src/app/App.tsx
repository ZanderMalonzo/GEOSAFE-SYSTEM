...
type Screen = "splash" | "login" | "register" | "home" | "report" | "map" | "alerts" | "profile" | "rescue" | "family";
...
function Field({
  ...
  return (
    <div>
      <label className="text-sm font-medium mb-1 block" style={{ color: "#212121" }}>{label}</label>
      <input
        type={type}
...
// ─── Login ────────────────────────────────────────────────────────────────────
function LoginScreen({ onLogin, onRegister }: { onLogin: (user: GeosafeUser) => void; onRegister: () => void }) {
    ...
    <div className="flex flex-col h-full bg-white overflow-y-auto">
      ...
      <div className="flex flex-col px-6 pt-8 gap-5 pb-8">
        ...
        <div className="text-center">
          ...
          <button onClick={onRegister} className="text-sm font-semibold" style={{ color: BLUE }}>Create Account</button>
        </div>
      </div>
    </div>
  );
...
function RegisterScreen({ onDone, onBack }: { onDone: () => void; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center gap-4 px-5 pt-6 pb-4 shadow-sm">
        <button onClick={step === 1 ? onBack : () => setStep(1)} className="p-2 rounded-full" style={{ backgroundColor: "#F7F9FC" }}>
          <ArrowLeft size={20} color="#212121" />
        </button>
        <div className="flex-1">
          <h2 className="text-lg font-bold" style={{ color: "#212121" }}>Create Account</h2>
          <p className="text-xs mt-0.5" style={{ color: "#616161" }}>Step {step} of 2</p>
        </div>
      </div>
      ...
      <div className="px-5 pt-4 pb-2">
        <div className="flex gap-2">
          <div className="flex-1 h-1.5 rounded-full" style={{ backgroundColor: BLUE }} />
          <div className="flex-1 h-1.5 rounded-full" style={{ backgroundColor: step === 2 ? BLUE : "#E0E0E0" }} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
        {step === 1 ? (
          <>
            <Field label="Full Name" value={form.fullName} onChange={set("fullName")} placeholder="Juan Dela Cruz" />
            <Field label="Birthday" value={form.birthday} onChange={set("birthday")} type="date" />
            <div>
              <label className="text-sm font-medium mb-2 block" style={{ color: "#212121" }}>Gender</label>
              <div className="flex gap-2">
                {["Male", "Female", "Other"].map(g => (
                  <button
                    ...
                    style={{
                      borderColor: form.gender === g ? BLUE : "#E0E0E0",
                      backgroundColor: form.gender === g ? BLUE + "12" : "white",
                      color: form.gender === g ? BLUE : "#616161",
                    }}
                  >
                    {g}
                  </button>
            ...
            <Field label="Phone Number" value={form.phone} onChange={set("phone")} type="tel" placeholder="+63 917 000 0000" />
            <Field label="Email Address" value={form.email} onChange={set("email")} type="email" placeholder="you@email.com" />
            <Field label="Home Address" value={form.address} onChange={set("address")} placeholder="Street, Barangay Bayanan" />
          </>
        ) : (
          <>
            ...
            <div className="rounded-2xl p-4 mt-1" style={{ backgroundColor: "#F7F9FC", border: "1px solid #E0E0E0" }}>
              <p className="text-xs font-semibold mb-3" style={{ color: BLUE }}>Emergency Contact</p>
              <div className="flex flex-col gap-4">
                <Field label="Contact Name" value={form.emergencyName} onChange={set("emergencyName")} placeholder="Maria Dela Cruz" />
                <Field label="Contact Number" value={form.emergencyPhone} onChange={set("emergencyPhone")} type="tel" placeholder="+63 917 000 0000" />
              </div>
            </div>
            <label className="flex items-start gap-3 cursor-pointer mt-1">
              <div
                className="w-5 h-5 mt-0.5 rounded flex items-center justify-center border-2 flex-shrink-0 transition-all"
                style={{ borderColor: agreed ? BLUE : "#BDBDBD", backgroundColor: agreed ? BLUE : "white" }}
                onClick={() => setAgreed(!agreed)}
              >
                {agreed && <CheckCircle size={12} color="white" />}
              </div>
              <span className="text-sm" style={{ color: "#616161" }}>
                I agree to the <span style={{ color: BLUE }} className="font-medium">Privacy Policy</span> and consent to location sharing during emergencies.
              </span>
            </label>
          </>
        )}
      ...
      <div className="px-5 py-4 shadow-[0_-2px_10px_rgba(0,0,0,0.06)]">
        {step === 1 ? (
          <button
            onClick={() => setStep(2)}
      ...
      <div className="flex flex-col gap-5 px-4 pb-4 pt-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => onNavigate("alerts")} className="text-xs font-medium" style={{ color: BLUE }}>See All</button>
          </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                {[
                  ...
                  { label: "Senior Citizens", val: seniors, set: setSeniors },
                ].map(({ label, val, set }) => (
                  <div key={label} className="rounded-xl p-3" style={{ backgroundColor: "#F7F9FC", border: "1px solid #E0E0E0" }}>
                    <p className="text-xs font-medium mb-2" style={{ color: "#616161" }}>{label}</p>
                    <div className="flex items-center gap-2">
                      <button
                      ...
                      >−</button>
                      <span className="text-base font-bold w-5 text-center" style={{ color: "#212121" }}>{val}</span>
                      <button
...
function ProfileScreen({ onLogout }: { onLogout: () => void }) {
  ...
  const menuItems = [
    ...
    { icon: LogOut, label: "Logout", danger: true, action: onLogout },
  ];
    ...
    <div className="flex flex-col h-full overflow-y-auto" style={{ backgroundColor: "#F7F9FC" }}>
      ...
      <div className="px-4 py-5 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {menuItems.map(({ icon: Icon, label, value, toggle, setToggle, danger, action }, i) => (
            <div
              key={label}
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "login": return (
        <LoginScreen
          onLogin={() => { setScreen("home"); setTabScreen("home"); }}
      ...
      case "register": return (
        <RegisterScreen
          ...
          onDone={() => { setScreen("home"); setTabScreen("home"); }}
        />
      );
      case "home": return <HomeScreen onNavigate={s => { setScreen(s); if (mainTabs.includes(s)) setTabScreen(s); }} />;
  ...
  return (
    <div className="flex items-center justify-center min-h-screen" style={{ backgroundColor: "#0D47A1" }}>
      {/* Mobile frame — full bleed on small screens */}
      <div
        className="relative flex flex-col overflow-hidden w-full h-full max-w-[390px] sm:h-[844px] sm:max-h-[100vh] sm:rounded-[40px]"
        style={{
          height: "100dvh",
